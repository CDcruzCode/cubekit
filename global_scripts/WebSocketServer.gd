...
@export var handshake_headers: PackedStringArray = PackedStringArray()
@export var supported_protocols: PackedStringArray = PackedStringArray()
@export var handshake_timout: int = 3000
...
func send(peer_id: int, message: String) -> int:
	...
	if type == TYPE_STRING && socket.get_ready_state() == socket.STATE_OPEN:
...
func poll() -> void:
	...
	for p:Variant in pending_peers:
...
func _connect_pending(p: PendingPeer) -> bool:
	if p.ws != null:
		...
		p.ws.poll()
		var state: WebSocketPeer.State = p.ws.get_ready_state()
		if state == WebSocketPeer.STATE_OPEN:
			var id: int = randi_range(2, 1 << 30)
			peers[id] = p.ws
